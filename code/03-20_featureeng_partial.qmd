...
fe_month_sum_wide <- fe_month_sum %>%
  ...
  dplyr::select(-name,-month_abb) %>% #this removes these columns; if left in, they cause an issue with pivot wider where some cells cannot pull data correctly, resulting in a data frame with way too many rows and many empty cells
...
#  scale_fill_viridis_c(option = "C") +
#  theme(legend.position = "none") 
#  facet_wrap(~name, scales = "free") # facet wrap applies single scale to all plots, ruins aesthetic
